...
export const ListingsTable = (props: IHotelTableProps) => {
  ...
  const [roomsData, setRoomsData] = useState<string[] | null>(null);
  ...
  const handleRoomsModal = (id: number) => {
    setHotelId(id);
        ...
        <Table>
          ...
          <TableBody>
            {items?.results?.map((hotel) => {
              return (
                <TableRow hover key={hotel.id}>
                  ...
                  <TableCell>
                    <Button
                      onClick={() => handleRoomsModal(hotel.id)}
                      sx={{ fontSize: "14px" }}
                      variant="text"
                    >
                      {hotel.room_count}
            ...
            {showRoomsModal && (
              ...
              >
                <RoomsMenu hotelId={hotelId as number} />
              </Modal>
            )}
          </TableBody>
        </Table>
...
export const RoomsMenu = ({ hotelId }: IRoomsMenuProps) => {
  const { data } = useGetRooomsByHotelQuery({ id: hotelId });
  console.log(data);
  return (
    <Box sx={{ ...roomsStyle }}>
